...
class StagePage(TaskSet):
    ...
    def play_rankings(self):
        ...
        response = self.client.get(
            "/api/rankings/play/?offset=0&limit=50", name="/api/rankings/play/")
        if response.ok:
    ...
    def rating_rankings(self):
        response = self.client.get(
            "/api/rankings/rating/?offset=0&limit=50", name="/api/rankings/rating/")
        if response.ok:
            self.stages = [r["stage"] for r in response.json()]
        # TODO: ページングの2ページ目以降には未対応
    ...
    def favorite_rankings(self):
        response = self.client.get(
            "/api/rankings/favorite/?offset=0&limit=50", name="/api/rankings/favorite/")
        if response.ok:
            self.stages = [r["stage"] for r in response.json()]
        # TODO: ページングの2ページ目以降には未対応
    ...
    def stage(self):
        ...
        self.client.get(
            "/api/stages/" + str(stage["id"]) + "?fields=all", name="/api/stages/:id?fields=all")
        self.client.get("/api/stages/" + str(
            stage["id"]) + "/rankings/score/keys", name="/api/stages/:id/rankings/score/keys")
        self.client.get("/api/stages/" + str(
            stage["id"]) + "/rankings/score/?offset=0&limit=50", name="/api/stages/:id/rankings/score/")
        # TODO: ページングの2ページ目以降には未対応
        # TODO: 期間を指定してのランキング表示には未対応
    ...
    def player_rankings(self):
        ...
        self.client.get("/api/rankings/player/?offset=0&limit=50",
                        name="/api/rankings/player/")
        # TODO: 期間を指定してのランキング表示には未対応
        # TODO: ページングの2ページ目以降には未対応
    ...
    def creator_rankings(self):
        self.client.get("/api/rankings/creator/?offset=0&limit=50",
                        name="/api/rankings/creator/")
        # TODO: ページングの2ページ目以降には未対応
        # TODO: 検索したユーザーの表示には未対応
